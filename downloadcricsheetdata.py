...
import sys, yaml, csv, glob, attr,os
import pandas as pd
...
class Match(object):
    ...
    win_by = attr.ib( default="")
    result_margin = attr.ib( default=0)
    ...
    def columns():
        return ["id", "season", "city", "date", "team1", "team2", "toss_winner",
                ...
                "win_by","result_margin", "player_of_match", "venue",
                "umpire1", "umpire2", "umpire3"]

    def values( self):
        return [self.id, self.season, self.city, self.date, self.team1, self.team2,
                self.toss_winner, self.toss_decision, self.result,
                self.dl_applied, self.winner, self.win_by,self.result_margin,self.player_of_match, self.venue,
...
class Delivery(object):
    ...
    is_wicket = attr.ib( default=0)
    fielder = attr.ib( default="")

    @staticmethod
    def columns():
        return ["id", "inning", "batting_team", "bowling_team", "over",
                "ball", "batsman", "non_striker", "bowler", "is_super_over",
                "wide_runs", "bye_runs", "legbye_runs", "noball_runs",
                "penalty_runs", "batsman_runs", "extra_runs", "total_runs",
                "player_dismissed", "dismissal_kind",'is_wicket', "fielder"]
    ...
    def values( self):
        return [self.match_id, self.inning, self.batting_team,
                ...
                self.dismissal_kind, self.is_wicket,self.fielder]

def write_match_info( match_data, match, matches_f,matchid):
    ...
    if "city" in info:
        ...
        match.city = "Sharjah"
    elif info["venue"] == "Dubai International Cricket Stadium":
        match.city = "Dubai"

    match.date = pd.to_datetime(info["dates"][0])
    ...
    if "result" in info["outcome"]:
        ...
        if "runs" in info["outcome"]["by"]:
            ...
            match.result_margin = win_by_runs
        else:
            match.win_by = "wickets"
            win_by_wickets = info["outcome"]["by"]["wickets"]
            match.result_margin = win_by_wickets

    if "player_of_match" in info:
        match.player_of_match = info["player_of_match"][0].replace("Mohsin Khan (2)","Mohsin Khan")
    ...
    match_venue = info["venue"].split(",")
    match.venue = match_venue[0]
...
def iswicket(player_dismissed):    
    if type(player_dismissed) != str:
        return 0
    elif ~(type(player_dismissed) != str) & (player_dismissed not in ['']):
        return 1
    else:
        return 0
...
def write_deliveries_info( match_data, match, deliveries_f):
    ...
    for inning in match_data["innings"]:
        ...
        if inning_number == 1:
            ...
            is_super_over = 1
        elif inning_number == 5: 
            inning_name = "Super Over 3"
            is_super_over = 1
        elif inning_number == 6: 
            inning_name = "Super Over 4"
            is_super_over = 1
        else: 
        ...
        for d_data in inning["deliveries"]:
            ...
            d = Delivery(match_id=match.id, inning=inning_number,
                    ...
                    is_wicket = iswicket(d_data.get( "wicket", {}).get( "player_out", "")),
                    fielder = d_data.get( "wicket", {}).get( "fielders", [""])[0])
            
            d.batsman = d.batsman.replace("Mohsin Khan (2)","Mohsin Khan")
            d.bowler = d.bowler.replace("Mohsin Khan (2)","Mohsin Khan")
            d.non_striker = d.non_striker.replace("Mohsin Khan (2)","Mohsin Khan")
            deliveries_f.writerow( d.values())

def process( match_data_f, matches_f, deliveries_f,matchid):
        
    match_data = yaml.full_load( match_data_f)
    ...
    match = Match()
    write_match_info( match_data, match, matches_f,matchid)
